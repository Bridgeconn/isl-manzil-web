...
interface BibleStore {
  ...
  initializeAvailableData: (shouldAutoSelect?: boolean) => Promise<void>;
...
const useBibleStore = create<BibleStore>()(
  persist(
    (set, get) => ({
      playbackSpeed: 1,
      ...
      checkUrlForBookChapter: () => {
        ...
        if (match) {
          ...
          const chapter = chapterStr.toLowerCase() === "introduction" || 
      ...
      updateUrlIfNeeded: () => {
        ...
        if (shouldUpdateUrl && selectedBook && selectedChapter) {
          ...
          const chapterLabel = selectedChapter.value === 0 
            ? "Introduction" 
      ...
      initializeAvailableData: async (shouldAutoSelect: boolean = true) => {
        set({ isLoading: true });
      ...
      getBibleVerseMarkerForBookAndChapter: async (
        ...
        try {
          ...
          const formattedVerseMarkerData: VerseMarkerType[] =
            parsedData.data.map((row: any, index) => {
              return {
                id: index + 1,
